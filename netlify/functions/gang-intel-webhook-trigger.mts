...
/**
 * Core function to send gang intel to Discord webhook
 */
async function sendGangIntelToDiscord(isManual: boolean = false): Promise<{ success: boolean; totalOrganizations: number; sentCount: number; error?: string }> {
    try {
        console.log(`Starting gang intel webhook job (${isManual ? 'manual trigger' : 'scheduled'})...`);
        ...
        if (gangsToSend.length === 0) {
            ...
            return { success: true, totalOrganizations: 0, sentCount: 0 };
        }
        ...
        for (let i = 0; i < gangsToSend.length; i += batchSize) {
            ...
            if (i === 0) {
                ...
                const triggerType = isManual ? '(Manual Report)' : '(Scheduled Report)';
                messagePayload.content = `📊 **Gang Intel Database Report** ${triggerType} - ${new Date().toLocaleDateString('en-US', {
                    weekday: 'long',
                    year: 'numeric',
                    month: 'long',
                    day: 'numeric',
        ...
        return {
            ...
            sentCount: 0,
            error: error instanceof Error ? error.message : 'Failed to send gang intel'
        };
    }
}

/**
 * Manual trigger endpoint for gang intel webhook
 * POST /api/intel/send-webhook
 */
export default async function handler(req: Request, context: Context) {
    ...
    const result = await sendGangIntelToDiscord(true);

    if (result.success) {
        return new Response(JSON.stringify({
            success: true,
            message: result.totalOrganizations === 0
                ? 'No gang intel to send'
                : `Successfully sent ${result.sentCount} of ${result.totalOrganizations} organizations to Discord`,
        ...
        return new Response(JSON.stringify({
            success: false,
            error: result.error || 'Failed to send gang intel'
        }), {
            status: 500,
            headers: { 'Content-Type': 'application/json' }
        });
